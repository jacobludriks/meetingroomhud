	...
	function wrap(text,position) {
		text.each(function() {
			...
			for (var i = 0; i < words.length; i++) {
				...
				if (i > 0)
					tspan.attr('x', position).attr('dy', '15');
			}
		})
	};
        ...
        d.forEach( function(datum, index){
          ...
          if (hasLabel) {
            gParent.append("text")
              ...
              .attr("transform", "translate("+ 0 +","+ (itemHeight * 0.5 + margin.top + (itemHeight + itemMargin) * yAxisMapping[index])+")")
          ...
          function getStackTextPosition(d, i) {
            if (stacked) {
              return (margin.top + (itemHeight + itemMargin) * yAxisMapping[index] + itemHeight * 0.75) - 40;
            }
